...
def plot_cluster_map(X, 
	...
	if n_clusters_ > 0:
		...
		if save_plot:
			...
			plt.title('Between %s and %s : %d clusters and % d points' % (round(range_low ,3), round(range_high,3), n_clusters_,n_clusters_points_))
			plt.axis(range_axis)
		
			F = plt.gcf()

			F.savefig(OUTPUT_CHARTS_DIR + '/' + data_column_i + '_quantile_group_' + str(quantile_group) +'.png' , dpi = (500))
			#plt.show()
			plt.clf
	return (labels)
...
class clustering:
	def expand_cluster_convexhull(self,df_input_name, df_cluster_name, df_cluster_expanded_name):
		...
		print(df_cluster_expanded_name)
		cluster_names_df = self.df_dict[df_cluster_name][['CLUSTER',self.sample_name_column,'PERCENTILE','CLUSTER_COUNT']].drop_duplicates(subset=None, keep='first', inplace=False)
		self.df_dict[df_cluster_expanded_name] = pd.DataFrame()
		for index, row in cluster_names_df.iterrows():
			#get the cluster
			cluster_points_df_i = self.df_dict[df_cluster_name].loc[(self.df_dict[df_cluster_name]['PERCENTILE'] == row['PERCENTILE']) & (self.df_dict[df_cluster_name]['CLUSTER'] == row['CLUSTER']) & (self.df_dict[df_cluster_name][self.sample_name_column] == row[self.sample_name_column])][['X','Y']]
			...
			contained_points = in_hull(self.df_dict[df_input_name][['X','Y']].values, hull_vertices)
			#contained_points
			...
			contained_points_df[self.sample_name_column] = [row[self.sample_name_column]] * len(contained_points_df.index)
			...
			#contained_points_df
			self.df_dict[df_cluster_expanded_name] = pd.concat([self.df_dict[df_cluster_expanded_name],contained_points_df], ignore_index = True)
		self.df_dict[df_cluster_expanded_name].ORIGINAL_CLUSTER.fillna('EXPANDED', inplace=True)
		self.df_dict[df_cluster_expanded_name]['CLUSTERED'] = ['CLUSTERED'] * len(self.df_dict[df_cluster_expanded_name].index)
		self.df_dict[df_cluster_expanded_name]['CLUSTER_RATIO'] = round(self.df_dict[df_cluster_expanded_name]['ORIGINAL_CLUSTER_COUNT'] / self.df_dict[df_cluster_expanded_name]['CLUSTER_COUNT'],2)
#Finds clusters and expanded clusters
	...
	def find_clusters(self, transformation_class):
		...
		df_input_name = transformation_class.df_input_name
		df_cluster_name =transformation_class.df_cluster_name
		df_output_name = transformation_class.df_output_name
		...
		if ("X" not in list(self.df_dict[df_input_name].columns)) and ("Y" not in list(self.df_dict[df_input_name].columns)):
			print("missing columns X and/or Y.  Cannot complete clustering")
			self.df_dict[df_cluster_name] = pd.DataFrame()
			self.df_dict[df_output_name] = self.df_dict[df_input_name].copy()
			return
		...
		for data_column_i in self.data_columns:
			...
			df = self.df_dict[df_input_name][['X','Y',data_column_i]]
			...
			if single_cpu:
				for quantile_group in range(0,len(percentile)-2,1):
					df_filtered = df.loc[(df[data_column_i] >= percentile[quantile_group]) & (df[data_column_i] <=  percentile[quantile_group+2])][['X','Y']]
					X = df_filtered[['X','Y']].values
					labels = plot_cluster_map(X, quantile_group, 
											  percentile[quantile_group], 
											  percentile[quantile_group+2], 
		...
		self.expand_cluster_convexhull( df_input_name, df_cluster_name = 'CLUSTER_TEMP', df_cluster_expanded_name = df_cluster_name)
		...
		if merge_to_input_df:
			...
			transformation_class = melt_class(df_in_name = df_input_name,
												df_out_name = df_output_name,
												value_vars = [],
												id_vars =  [],
			...
			transformation_class = merge_data_frames( df_name_left= df_output_name, 
													 df_name_right= df_cluster_name, 
													 df_out = df_output_name,
													 on_columns=[], 
													 how = 'outer')
			self.merge_data_frames(transformation_class)
			self.df_dict[df_output_name].CLUSTER.fillna(-1, inplace=True)
			self.df_dict[df_output_name].PERCENTILE.fillna('NONE', inplace=True)
			self.df_dict[df_output_name].ORIGINAL_CLUSTER_COUNT.fillna(0, inplace=True)
			self.df_dict[df_output_name].CLUSTER_COUNT.fillna(-1, inplace=True)
			self.df_dict[df_output_name].ORIGINAL_CLUSTER.fillna('NONE', inplace=True)
			self.df_dict[df_output_name].CLUSTERED.fillna('NONE', inplace=True)
			self.df_dict[df_output_name].CLUSTER_RATIO.fillna(0, inplace=True)
			self.df_dict[df_output_name]['EXCLUDE'] = self.df_dict[df_output_name]['CLUSTER'].apply(lambda x: True if x > -1 else False)
	...
	def exclude_clustered_data(self, transformation_class):
		...
		if filter_clusters:
			self.df_dict[df_out_name] = self.df_dict[df_out_name].loc[~((self.df_dict[df_out_name]['ORIGINAL_CLUSTER']== 'ORIGINAL') & (self.df_dict[df_out_name]['ORIGINAL_CLUSTER_COUNT']>= min_original_cluster_size ) & (self.df_dict[df_out_name]['CLUSTER_RATIO']>=min_cluster_ratio))]
		if filter_expanded:
			self.df_dict[df_out_name] = self.df_dict[df_out_name].loc[~((self.df_dict[df_out_name]['ORIGINAL_CLUSTER']== 'EXPANDED') & (self.df_dict[df_out_name]['ORIGINAL_CLUSTER_COUNT']>= min_original_cluster_size ) & (self.df_dict[df_out_name]['CLUSTER_RATIO']>=min_cluster_ratio))]
		if unstack:
			...
			#self.descriptor_columns = set(self.df_dict[df_output_name].columns) - set(self.data_column_class)		
		return
class find_clusters:
	def __init__( self,
				  df_input_name = 'df',
