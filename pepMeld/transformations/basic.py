class basic_transformations:
	def log_transformation(self,transformation_class = None):
		...
		if not self.logtransformed:
			import numpy as np
			if df_out_name != df_in_name:
				self.df_dict[df_out_name] = self.df_dict[df_in_name].copy(deep = True)
			if base ==2:
				self.df_dict[df_out_name][transform_columns] = self.df_dict[df_in_name][transform_columns].apply(np.log2)
			elif base ==10:
				 self.df_dict[df_out_name][transform_columns] = self.df_dict[df_in_name][transform_columns].apply(np.log)
			else:
				self.df_dict[df_out_name][transform_columns] = self.df_dict[df_in_name][transform_columns].apply(np.log2)
				self.df_dict[df_out_name][transform_columns] = self.df_dict[df_in_name][transform_columns].div(np.log2(base))
			self.logtransformed = True
			print('Data log '+ str(transformation_class.base) +' Transformed on: [' + ', '.join(transform_columns) + ' ]')
		else:
			print('Already Transformed, Please Reload the data to transform or override self.logtransformed=False')
	...
	def median_group_by(self, transformation_class):
		...
		if not group_by_columns:
			print('WARNING: groupby columns all missing median transform did not take place: ' + ';'.join(group_by_columns))
			return
		if df_out_name == df_in_name:
			print('Warning: Future median evaluations cannot take place-output and input same.')
		self.df_dict[df_out_name] = self.df_dict[df_in_name].groupby(group_by_columns, as_index=False)[self.data_columns].median()
	...
	def get_subtract_columns_dictionary(self,
		...
		if (subtract_column not in self.df_dict['meta'].columns) or (sample_name not in self.df_dict['meta'].columns):
			...
			raise RuntimeError("Missing Required Column in Meta Data: " + error_message + ' \n Out of Meta data columns: ' + ", ".join( self.df_dict['meta'].columns) + ' ; \n Script has terminated')
		subtract_name_dict = {key: list(value)[0] for key, value in self.df_dict['meta'].groupby(sample_name)[self.sample_name_column]}
		subtract_dict = { ';;'.join([subtract_name_dict[y] for y in key.split(';')]) : [subtract_name_dict[x] for x in value] for key,value in self.df_dict['meta'].groupby(subtract_column)[sample_name]} 
	...
	def subtract_columns(self,transformation_class):
		...
		if not self.logtransformed:
			print('Warning: Data must first be transformed, or override by setting self.logtransformed to True')
			return
		...
		for subtract_column, column_list in subtract_dict.items(): 
			...
			if ';;' in subtract_column:
				...
				# print(self.df_dict)
				self.df_dict[df_in_name][subtract_column] = self.df_dict[df_in_name].apply(lambda row: correct_multiple_controls_3(row[subtract_column_list[0]], 
																																	  row[subtract_column_list[1]]), axis=1)

			if subtract_column in self.df_dict[df_in_name].columns:
				self.df_dict[df_out_name][column_list] = self.df_dict[df_in_name][column_list].sub(self.df_dict[df_in_name][subtract_column], axis=0)
	...
	def rolling_median(self,transformation_class):
		...
		self.df_dict[df_out_name][self.data_columns] = self.df_dict[df_in_name].groupby(group_by_columns)[self.data_columns].transform(lambda x: x.rolling(rolling_window).median())
	...
	def evaluate_medians(self):
		...
		df_melt_skewed = df_melt.loc[((df_melt['value_4'] > 1.5) & (df_melt['value_median']< 1.5) & (df_melt['value_4_median'] > 0.75))| 
