...
def check_required_fields(args):
	...
	require_inputs = [ 'input_path', 
					  'metadata_path',
					  'data_output_dir',
					  'charts_output_dir']
	for args_i in require_inputs:
		if args[args_i] is None:
			raise('{0} must be declared'.format(args_i))
		...
		if args[args_i] is not None:
			if args_i[-4:]=='_sep':
				if len(args[args_i]) != 1:
					raise('separater must be one character: {0}'.format(args[args_i]))
		else:
			#set default
			args[args_i] = '\t'
			...
			if args_i[-5:]=='_path':
				...
				for path_i in path_list:
					if not os.path.isfile(path_i):
						raise('file cannot be found for {0} : {1}'.format(args_i, args[args_i]))
